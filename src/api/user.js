 ...
 * 文章列表
 * */
export function getArticles(rq) {
  return request.get("/baiwen", { query: rq });
}
/*
 * 文章列表
 * */
export function getArticlesDetail(rq) {
  return request.get("/baiwen_details", { id: rq });
}

/*
 * 获取其他用户信息
 * */
export function getUidUserInfo(uid, phone) {
  return request.get("/ouser", { uid: uid, phone: phone });
}

/*
 * 获取其他用户信息
 * */
export function fromTransfer(from_uid, price) {
  return request.post("/extract/transfer", {
    from_uid: from_uid,
    price: price
  });
}
 ...
 * 转佣金
 * */
export function fromFommission(price) {
  return request.post("/extract/fommission", {
    price: price
  });
}