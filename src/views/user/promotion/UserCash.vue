  ...
  <div class="cash-withdrawal">
    ...
    <div class="wrapper">
      <div :hidden="currentTab !== 0" class="list">
        ...
        <div class="tip">
          当前可提现金额: {{ commissionCount }}
          <span style="color: red;">手续费{{ cash ? cash : 0 }}%</span>
        </div>
  ...
  methods: {
    ...
    submitted() {
      let bankname = this.post.bankname,
        ...
        that = this;
      if (money == "") {
        return that.$dialog.message("提现金额不能为空");
      }

      if (parseFloat(money) < parseFloat(that.minPrice)) {
        return that.$dialog.message("最低提现金额" + that.minPrice);
      }
      if (parseFloat(money) > parseFloat(that.commissionCount)) {
        return that.$dialog.message("余额不足以提现");
      }
      money = parseFloat(Math.floor(money / 10) * 10);
      that.$dialog.confirm({
        title: "提示",
        mes: "系统自动取整为提现 " + money + " 元",
        opts: [
          {
            txt: "取消",
            color: false,
    ...
    async getMoney(money, bankname, name, cardnum, alipay_code, weixin) {
      console.log("money");
      var that = this;
