<template>
  <div class="ChangePassword">
    ...
    <div class="confirmBnt bg-color-red" @click="confirms">确定</div>
...
<script>
import { getUidUserInfo, fromTransfer, fromFommission } from "@api/user";
export default {
  name: "Transfer",
  data: function() {
    return {
  ...
  methods: {
	  confirms(){
		  var that = this;
		  fromFommission(that.price).then(res=>{
			  if(res.status == 200){
				  that.$dialog.message(res.msg);
				  setTimeout(function(){
				  		that.$router.replace({ path: "/user/user_promotion" });
				  },1000)
			  }else{
				  that.$dialog.message(res.msg);
			  }
