...
function getAuthUrl(appId) {
  const redirect_uri = encodeURIComponent(
    `${location.origin}`
    // `http://shop.paiyipai.vip/#/`
    // encodeURIComponent(
    // 	encodeURIComponent(
    // 		cookie.has(BACK_URL) ? cookie.get(BACK_URL) : location.pathname + location.search
    // 	)
    // )
  );
  // return false;
...
export function openShareAll(config) {
  ...
  return new Promise(resolve => {
    if (instance && wechatObj) {
      instance.ready(() => {
        wechatObj.shareOnChat(config);
        wechatObj.shareOnMoment(config);
        resolve();
      });
    } else {
      getWechatConfig().then(res => {
        const _wechat = WechatJSSDK(res.data);
        _wechat.shareOnChat(config);
        _wechat.shareOnMoment(config);
        resolve();
      });
    }
  });
